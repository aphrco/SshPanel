    ...
    version="1.0"
    date="2023-10-16"
    title="SSH Panel irvps.shop v$version"

    #FILE ADDRESS
    USER_FILEALL="$(pwd)/userall.txt"
    USER_FILE="$(pwd)/users.txt"
    SETTING_FILE="$(pwd)/setting.txt"
...
#SETTING SET
    read_setting() {
        if [ -f "$SETTING_FILE" ]; then
            ...
            DEFAULT_USERNAME_SUFFIX=$(grep "^Suffix:" "$SETTING_FILE" | awk '{print $2}')
        else
            DEFAULT_USERNAME_SUFFIX="irvps-"  
            host= ip a
            port="22"
            port_udpqw="7301"
        fi
...
#User_Manager
    ...
    add_user() {
        ...
        echo "Enter your (Telegram User/Name User):"
        read telegramuser
        echo "Enter the Expiration days for the User(Day Number)"
    ...
    delete_user() {
        clear
        echo "Select User Of List For Delete:"
        ...
        if [[ "$user_number" =~ ^[0-9]+$ ]]; then
            ...
            if [ -n "$username" ]; then
                clear
                read -p "Are you sure you want to delete user $username? (yes/n): " confirm_delete
                if [[ "$confirm_delete" == "y" || "$confirm_delete" == "Y" || "$confirm_delete" == "Yes" ]]; then
    ...
    change_user_password() {
        ...
        read -p "Are you sure you want to Change password for $username? (yes/n): " confirm_password
        if [[ "$confirm_password" == "y" || "$confirm_password" == "Y" || "$confirm_password" == "Yes" ]]; then
        if [ -n "$username" ]; then
    ...
    extend_user_expiration() {
        ...
        read -p "Are you sure you want to Change Expiration Date for $username? (yes/n): " confirm_expiration
        if [[ "$confirm_expiration" == "y" || "$confirm_expiration" == "Y" || "$confirm_expiration" == "Yes" ]]; then
    ...
    change_username_suffix() {
        ...
        read default_suffix
        sed -i "s/^Suffix: .*/Suffix: $default_suffix/" "$SETTING_FILE"
    ...
    change_ssh_port() {
        ...
        read new_port
        sed -i "s/^Port: .*/Port: $new_port/" "$SETTING_FILE"
